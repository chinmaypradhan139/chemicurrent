...
cols = ["Er","icsq (pA)","% ierror","iintercept","ir (pA)","% rerror","rintercept","\n"]
...
for i in range(1,folds_in_tar+1):
    ...
    for ifil in ifile:
        ...
        slopi=popt[0]*pA
        errp=err[0]*100/popt[0]
        slope.append(slopi)
        perr.append(errp)
        inter.append(popt[1])


    out=[str(Er),str(slope[0]),str(perr[0]),str(inter[0]),str(slope[1]),str(perr[1]),str(inter[1]),"\n"]
    fk = open(outputfil, 'a')
    fk.write(",".join(out))
    fk.close()
