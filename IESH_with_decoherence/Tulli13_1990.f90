...
do while ((n.le.Ne).and.(i.le.Hi))
    call random_number(rnd4)
    if (fdist(H(i,i))>rnd4) then
        diabstate(n)=i
        n=n+1
    end if
    i=i+1
...
do i=1,Ne
   do while(diabstate(i)==0) 
     do j=2,Hi
        if(.not.(any(j==diabstate))) then
            call random_number(rnd4)
            if (fdist(H(j,j))>rnd4) then
                diabstate(i)=j
            end if
        end if
     enddo
...
do while (time.le.total_time)
   call two_state_dynamics(outstate)
!    call simple_dynamics(outstate,5)
    if (outputs(7)==1) call calchemcurr(real(outstate),barrier,7)
    if (outputs(3)==1) tdata(int(time/dtc),3)=outstate(1)
   time=time+dtc
enddo
